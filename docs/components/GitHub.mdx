...
<CardGrid>
  ...
  <LinkCard title="Get Billing Usage" href="#get-billing-usage" description="Retrieve billable GitHub Actions usage for the organization" />
...
- Repository selection is for reference/tracking only; API returns org-wide totals

### Configuration

- **Repositories** (optional, multiselect): Select one or more repositories to track. These will be included in the output for reference (max 5) and stored in node metadata with full repository details (ID, name, URL). Note: The usage data returned is organization-wide and not filtered by repository.

### Node Metadata

The component stores repository information in node metadata for display in the workflow canvas:
- Repository ID, name, and URL for each selected repository (max 5)
- This makes it easy to see which repositories are being tracked at a glance
...
**Note**: Breakdown is by runner OS type, not by individual workflow.
