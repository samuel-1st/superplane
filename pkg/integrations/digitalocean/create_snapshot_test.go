...
func Test__CreateSnapshot__Setup(t *testing.T) {
	...
	t.Run("missing snapshotName returns error", func(t *testing.T) {
		err := component.Setup(core.SetupContext{
			Configuration: map[string]any{
				"dropletID": "98765432",
			},
		...
		require.ErrorContains(t, err, "snapshotName is required")
	})
...
func Test__CreateSnapshot__Execute(t *testing.T) {
	...
	t.Run("successful snapshot dispatch -> stores metadata and schedules poll", func(t *testing.T) {
		...
		require.True(t, ok)
		assert.Equal(t, 36804752, metadata["actionID"])
			...
			Responses: []*http.Response{
				{
					...
					Body:       io.NopCloser(strings.NewReader(`{"id":"unprocessable_entity","message":"Cannot take snapshot while droplet is off"}`)),
				},
			},
		}
...
func Test__CreateSnapshot__HandleAction(t *testing.T) {
	...
	t.Run("action completed -> emits result", func(t *testing.T) {
		...
		metadataCtx := &contexts.MetadataContext{
			Metadata: map[string]any{"actionID": 36804752},
		}
		...
		assert.Equal(t, "digitalocean.droplet.snapshot.created", executionState.Type)
	})
